...
blink {
    name.set("SymphonyLevel")
    version.set(project.version.toString())
    description.set("为 Symphony 提供等级数据的外部插件")
    authors.set(listOf("17Artist"))
    packageName.set("priv.seventeen.artist.symphony.level")
    apiVersion.set("1.18")
    logPrefix.set("§6♬ §b等级提供者")
...
val verifyRuntimeBoundary by tasks.registering {
    ...
    doLast {
        ...
        ZipFile(published).use { zip ->
            ...
            check(forbidden.isEmpty()) { "等级提供者 JAR 包含不应打包的共享类：${forbidden.take(20)}" }
            val descriptor = zip.getInputStream(requireNotNull(zip.getEntry("plugin.yml"))).bufferedReader().use { it.readText() }
            check("depend:" in descriptor && "Symphony" in descriptor) { "等级提供者插件必须依赖 Symphony" }
            check(entries.any { it == "priv/seventeen/artist/symphony/level/blink/bootstrap/KotlinBootstrap.class" }) {
                "等级提供者插件缺少插件内部重定位后的 Blink 运行库"
            }
        }
    }
}
