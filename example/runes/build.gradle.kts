...
blink {
    name.set("SymphonyRunes")
    version.set(project.version.toString())
    description.set("接入 Symphony 的外部符文系统")
    authors.set(listOf("17Artist"))
    packageName.set("priv.seventeen.artist.symphony.runes")
    apiVersion.set("1.18")
    logPrefix.set("§6♬ §d符文")
    depend.set(listOf("Symphony"))
    softDepend.set(listOf("SymphonyLevel"))
...
dependencies {
    ...
    testImplementation(project(":symphony-api"))
...
val verifyRuntimeBoundary by tasks.registering {
    ...
    doLast {
        ...
        ZipFile(published).use { zip ->
            ...
            check(forbidden.isEmpty()) { "符文插件 JAR 包含不应打包的共享类：${forbidden.take(20)}" }
            val descriptor = zip.getInputStream(requireNotNull(zip.getEntry("plugin.yml"))).bufferedReader().use { it.readText() }
            check("depend:" in descriptor && "Symphony" in descriptor) { "符文插件必须依赖 Symphony" }
            check(entries.any { it == "priv/seventeen/artist/symphony/runes/blink/bootstrap/KotlinBootstrap.class" }) {
                "符文插件缺少插件内部重定位后的 Blink 运行库"
            }
        }
    }
}
