...
package priv.seventeen.artist.symphony.runes.command
...
class LocalizedCommand(
    ...
    private fun sendHelp(sender: CommandSender, label: String): Boolean {
        ...
        visibleEntries(sender).forEach { entry ->
            sender.sendMessage(t("command-system.help-entry", "usage" to usage(label, entry), "description" to entry.description))
        }
        return true
    }
...
object LocalizedCommandRegistrar {
    ...
    private class Registration(private val map: CommandMap, private val command: LocalizedCommand) : AutoCloseable {
